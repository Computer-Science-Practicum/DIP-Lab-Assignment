...
class DFT(Matrix):
    ...
    def inverse(self) -> Image:
        img = Image()
        img.data = np.fft.ifft2(self.data).real
        return img
    
    def amplitude(self) -> Image:
        img = Image()

        img.data = np.abs(self.data)
        # img.data = np.abs(np.log(self.data))
...
class ImageFilters:
    ...
    def edge_detection(image: Image) -> tuple:
        sobel_x = ImageFilters.convolve(image, ImageFilters.sobel_x)
        sobel_y = ImageFilters.convolve(image, ImageFilters.sobel_y)
        return (sobel_x, sobel_y)
    
    @staticmethod
    def laplacian_filter(image: Image) -> np.ndarray:
